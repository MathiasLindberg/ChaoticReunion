...
public abstract class UpstreamMessage {
    ...
    public UpstreamMessage(BootloaderMessageType msgType)
        : base()
    {
...
virtual
    ...
    internal static UpstreamMessage parse(BinaryReader __buffer) {
        // Extract fields:
        ...
        case BootloaderMessageType.ERASE_FLASH:
            return EraseFlashResp.parse(__buffer);
        case BootloaderMessageType.PROGRAM_FLASH:
            return ProgramFlashDone.parse(__buffer);
        case BootloaderMessageType.INITIATE_LOADER:
            return InitiateLoaderResp.parse(__buffer);
        case BootloaderMessageType.GET_INFO:
            return GetInfoResp.parse(__buffer);
        case BootloaderMessageType.GET_CHECKSUM:
            return GetChecksumResp.parse(__buffer);
        case BootloaderMessageType.GET_FLASH_STATE:
            return GetFlashStateResp.parse(__buffer);
        default: throw new ArgumentException("Invalid value of msgType: "+msgType);
        } // switch
    }

    public abstract void visitWith<T>(UpstreamMessage_Visitor<T> visitor, T arg);
    ...
    public static UpstreamMessage parse(byte[] data) {
        BinaryReader buffer = new BinaryReader(new MemoryStream(data));
        UpstreamMessage result = parse(buffer);
